...
def run_viewer(source: Any, *, headless: bool = False, max_frames: int | None = None,
    ...
    """
    import cv2
    # rectify and depth are developed but not published — they have never been
    # run against a camera. The viewer's left/right, exposure, gain and
    # illuminator paths do not need them, so their absence disables two keys
    ...
    state = ViewerState(has_calib=calib is not None and _stereo_available)
    out = Path(out_dir) if out_dir is not None else Path("captures")
    keys = iter(key_feed) if key_feed is not None else None
    _EXHAUSTED = object()
    maps = None
    dparams = DepthParams(**(depth_params or {})) if _stereo_available else None
    depth_valid = float("nan")
    if calib is not None and _stereo_available:
        maps = stereo_rectify_maps(calib)
    elif calib is not None:
        state.messages.append(
            "rectify/depth are not in this build: they have not been run against the camera")
    n = 0
    size_warned = False
